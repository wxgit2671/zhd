...
public class Page<T> implements Iterable<T> {
    ...
    public Page<T> useSubList(Page<T> page, List<T> targetList, int pageSize) {
        ...
        int end = page.getPageNo() * page.getPageSize() > size ? size : page.getPageNo() * page.getPageSize();
        if (start < size) {
            List<T> list = targetList.subList(start, end);
            page.setResult(list);
        } else {
            page.setResult(null);
        }
     ...
     * 逻辑分页
     *
     * @param targetList 查询List
     * @return page
     */
    public Page<T> useSubList(List<T> targetList) {
        if (CollectionUtils.isEmpty(targetList)) {
            return this;
        }
        int size = targetList.size();
        ...
        int start = (this.getPageNo() - 1) * this.getPageSize();
        int end = this.getPageNo() * this.getPageSize() > size ? size : this.getPageNo() * this.getPageSize();
        if (start < size) {
            List<T> list = targetList.subList(start, end);
            this.setResult(list);
        } else {
