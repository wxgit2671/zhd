package com.wxtest.demo.common.tablestore.utils;
/**
...
public class CreatePrimaryKeyUtils {
    ...
    /**
     * 创建以userId,phone为主键名的主键
     * @param partitionKey
     * @param parameterKey
     * @return
     */
    public static PrimaryKey createOldPrimaryKey(String partitionKey, String parameterKey) {
        // 首先对入参进行判断 若为空直接返回
        if (StringUtils.isBlank(partitionKey) || StringUtils.isBlank(parameterKey)) {
            logger.error("分区键{}和主键{}中存在空值,主键构造失败", partitionKey, parameterKey);
            return null;
        ...
        //对分区键MD5
        partitionKey = md5Hex(partitionKey);
        logger.info("md5==partitionKey:{}",partitionKey);
        // 构造主键
        PrimaryKeyBuilder primaryKeyBuilder = PrimaryKeyBuilder.createPrimaryKeyBuilder();
        primaryKeyBuilder.addPrimaryKeyColumn("userId", PrimaryKeyValue.fromString(partitionKey));
        primaryKeyBuilder.addPrimaryKeyColumn("phone", PrimaryKeyValue.fromString(parameterKey));
        return primaryKeyBuilder.build();
    }
